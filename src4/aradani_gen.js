...
for (let g = 01; g < 01000; g++) {
  ...
  if (mc === 1) {
    ...
    roll1(1, ac);
    roll1(2, nc);
    log(`  ο[α - 1].c(ο, α - 1, ρ - 3, σ);`);
  } else {
    roll(0, oc, mc);
    roll(1, ac, mc);
    roll(2, nc, mc);
    log(`  ο[α - 1].c(ο, α - 1, ρ - ${(mc + 1) * 3}, σ);`);
  }
  log(`}`);
}
log(`N(dot) { ο[α - 1].c(ο, α - 1, ρ, σ); }
n_t Τ[01000] = {
  dot,`);
for (let g = 01; g < 01000; g++) {
  const nc = (g >> 6) & 7;
...
function roll(ray, xc, mc) {
  let i = 0;
  for (; i < xc; i++) log(`  ο[ρ - ${(i + 1) * 3 - ray}].v = ο[--α].v;`);
...
function roll1(ray, xc) {
  log(`  ο[ρ - ${3 - ray}].v = ${xc ? `ο[--α].v` : rni(ray, 1)};`);
}
function rni(r, i) {
  const d = ["Gor", "God", "Got"];
...
function UnrollNar(c, ray) {
  ...
  for (let i = 0; i < c - 1; i++) log(`  ο[α++].v = ο[ρ + ${ray}].v; ρ += 3;`);
  log(`  C(${ray});`);
  log(`}`);
}
function log(...args) {
