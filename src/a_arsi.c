...
SP(ayr_not) {
  if(α == 3) {
    R(n_t, e); R(void*, a); R(const char*, n); print("%s %p %p\n", n, a, e);
  } else Α(god, ο[7].p, 1, os_queue_n) O;
}
SP(ayr_oor) { R(p_t*, oο); ο[7].p = oο; Α(0, sgod) O; } 
...
SP(init) {
  ...
  oο[--oρ].c = os_next;
  oο[--oρ].c = ayr_oor;
  oο[Φ].p = oο, oο[Ρ].Q = oρ, QUEUE_INIT((QUEUE *)&oο[Ψ]), oο[Ν].cs = "α";
  C(1);
}
