#include <raylib.h>
/*
  console.log(funs.split(/RLAPI /).map(l => {
    const seg = l.split(/\(/);
    if(seg.length <= 1) return seg.join('(')
    const seg0 = seg[0].split(/ /);
    seg0[seg0.length-1] = '(*'+seg0[seg0.length-1]+')('
    return  seg0.join(' ') + seg.slice(1).join('(')
  }).join(''))
*/
//------------------------------------------------------------------------------------
