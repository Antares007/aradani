...
#define QUEUE_ADD(h, n)                                                        \
  (QUEUE_PREV_NEXT(h) = QUEUE_NEXT(n), QUEUE_NEXT_PREV(n) = QUEUE_PREV(h),     \
   QUEUE_PREV(h) = QUEUE_PREV(n), QUEUE_PREV_NEXT(h) = (h))
#define QUEUE_SPLIT(h, q, n)                                                   \
  (QUEUE_PREV(n) = QUEUE_PREV(h), QUEUE_PREV_NEXT(n) = (n),                    \
   QUEUE_NEXT(n) = (q), QUEUE_PREV(h) = QUEUE_PREV(q),                         \
   QUEUE_PREV_NEXT(h) = (h), QUEUE_PREV(q) = (n))
#define QUEUE_MOVE(h, n)                                                       \
  (QUEUE_EMPTY(h) ? QUEUE_INIT(n) : QUEUE_SPLIT(h, QUEUE_HEAD(h), n))
