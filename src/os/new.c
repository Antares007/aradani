...
#define SR(R, AND)                                                            \
  S(s##R) {                                                                   \
    if ((ο[ρ + 3].Q = !ο[ρ + 3].Q)) ο[ρ + 4 + R].c(T());                      \
    else (AND), ο[Φ].p[ο[Φ].p[Ρ].Q + R].c(σ, α, ο[Φ].p, ο[Φ].p[Ρ].Q);         \
  }
...
N(os_new) {
  ...
  R(n_t, r1);
  R(n_t, r0);
  Α(s0, s1, s2, 1, r0, r1, r2, ο[Φ].p, 512, "os_new", 7, os_new_wpsn) O;
  //Α(r0, r1, r2, ο[Φ].p, 512, "os_new", 3, os_new_wpsn) O;
}
